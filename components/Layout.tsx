...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="h-screen flex bg-[#F8F9FC] text-slate-900 font-sans overflow-hidden">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-16 flex flex-col items-center py-2 border-r border-slate-200/60 bg-white/95 backdrop-blur-md transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        <div className="flex flex-col items-center mb-2 gap-1 mt-8 sm:mt-0">
            <Link to="/dashboard" className="w-8 h-8 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-lg flex items-center justify-center shadow-lg shadow-indigo-500/30 hover:scale-105 transition-transform duration-300">
              <Video className="text-white w-4 h-4" />
            </Link>
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest scale-90">助手</span>
        </div>

        <nav className="flex-1 flex flex-col gap-0.5 w-full px-1.5 items-center">
          
          {/* Big Add Button - High End Gradient */}
          <button
            onClick={handleCreateClick}
            className="w-8 h-8 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-lg flex items-center justify-center shadow-lg shadow-violet-500/30 hover:shadow-violet-500/50 hover:scale-105 transition-all duration-300 group mb-1"
            title="新建项目"
          >
          ...
          <Link
            to="/dashboard"
            className={`flex flex-col items-center justify-center py-1 px-1 w-full rounded-lg transition-all gap-0.5 duration-300 ${
              isActive('/dashboard') 
                ? 'bg-violet-50 text-violet-700 shadow-sm' 
                : 'text-slate-400 hover:bg-slate-50 hover:text-slate-700'
            }`}
          >
            <LayoutDashboard className={`w-4 h-4 ${isActive('/dashboard') ? 'stroke-[2.5px]' : 'stroke-2'}`} />
            <span className="text-[9px] font-bold tracking-wide scale-90">项目</span>
          </Link>
          ...
          >
            <ImageIcon className={`w-4 h-4 ${isActive('/images') ? 'stroke-[2.5px]' : 'stroke-2'}`} />
            <span className="text-[9px] font-bold tracking-wide scale-90">生图</span>
          </Link>
          ...
          >
            <Lightbulb className={`w-4 h-4 ${isActive('/inspiration') ? 'stroke-[2.5px]' : 'stroke-2'}`} />
            <span className="text-[9px] font-bold tracking-wide scale-90">灵感</span>
          </Link>
          ...
          >
            <Type className={`w-4 h-4 ${isActive('/ai-titles') ? 'stroke-[2.5px]' : 'stroke-2'}`} />
            <span className="text-[9px] font-bold tracking-wide scale-90">标题</span>
          </Link>
          ...
          >
            <Settings className={`w-4 h-4 ${isActive('/settings') ? 'stroke-[2.5px]' : 'stroke-2'}`} />
            <span className="text-[9px] font-bold tracking-wide scale-90">设置</span>
          </Link>

          <div className="mt-auto w-full flex flex-col gap-0.5 border-t border-slate-100 pt-2">
             <button
                onClick={handleUpload}
                disabled={!!syncing}
                className={`flex flex-col items-center justify-center py-1 px-1 w-full rounded-lg transition-all gap-0.5 text-slate-100 disabled:opacity-50 ${
                    hasUnsavedChanges
            ...
            >
                {syncing === 'upload' ? <Loader2 className="w-3.5 h-3.5 animate-spin text-white" /> : <CloudUpload className="w-3.5 h-3.5 stroke-2" />}
                <span className="text-[8px] font-bold tracking-wide scale-90">上传</span>
            </button>
            <button
                onClick={handleDownload}
                disabled={!!syncing}
                className="flex flex-col items-center justify-center py-1 px-1 w-full rounded-lg transition-all gap-0.5 text-slate-400 hover:bg-emerald-50 hover:text-emerald-600 disabled:opacity-50"
                title="从云端下载数据"
            >
                {syncing === 'download' ? <Loader2 className="w-3.5 h-3.5 animate-spin text-emerald-500" /> : <CloudDownload className="w-3.5 h-3.5 stroke-2" />}
                <span className="text-[8px] font-bold tracking-wide scale-90">下载</span>
            </button>
          </div>

          <button
            onClick={handleLogout}
            className="mb-1 flex flex-col items-center justify-center py-1 px-1 w-full rounded-lg transition-all gap-0.5 duration-300 text-slate-400 hover:bg-rose-50 hover:text-rose-600"
            title="退出登录"
          >
            <LogOut className="w-4 h-4 stroke-2" />
            <span className="text-[9px] font-bold tracking-wide scale-90">退出</span>
          </button>
        </nav>
      </aside>
